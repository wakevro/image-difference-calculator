...
def main():
    ...
    csv_filename = "results/synchronous-results.csv"

    with open(csv_filename, mode="w", newline="") as csv_file:
        csv_writer = csv.writer(csv_file)

        # Add a title line describing the type of processing
        csv_writer.writerow(["Processing Type", "Synchronous"])
        ...
        for main_image_path in main_image_paths:
            ...
            sorted_results = sorted(results)  # Sort results by ID
            csv_writer.writerows(
                [(main_image_id, diff, elapsed_time) for _, diff in sorted_results]
            )
            all_results.extend(
                [(main_image_id, _, diff, elapsed_time) for _, diff in sorted_results]
            )
